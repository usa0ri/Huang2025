...
def prep_data(
    ...
    for a in ref.exp_metadata.df_metadata['align_files']:
        ...
        obj.decode()
        df,_ = obj.make_df(tr_cnt_thres=-1,is_frame=True,is_cds=True,is_seq=True)
...
def read_stat(
    ...
    i = 0
    for k,(( _,row ),readfile_name) in enumerate(zip(ref.exp_metadata.df_metadata.iterrows(),readfile_names)):
...
def filter_reads_save_data(
    ...
    for s in smpls:
        ...
        df = pd.DataFrame()
        for tr,df_now in dict_tr.items():
...
def extract_cds(
    ...
    fout = gzip.open(ref.ref_dir / 'cds.fa.gz','wt')
    for tr,seq in ref.ref_cdna.items():
        if tr not in ref.annot.annot_dict.keys():
            continue
        ref_cds = seq[ref.annot.annot_dict[tr]['start']:ref.annot.annot_dict[tr]['stop']+3]
        fout.write(f'>{tr}\n{ref_cds}\n')
    fout.close()
